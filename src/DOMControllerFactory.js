function DOMControllerFactory() {
  ...
  function drawShip(name, startX, startY, isVertical) {
    let img = new Image();
    ...
    if (ship.isSunk()) {
      if (isPlayer) {
        setInfoContainerText(
          `${name} fires at (${x},${y}) and sinks your ${ship.name}!`
        );
      } else {
        setInfoContainerText(
          `Direct hit! you sank the Computer's ${ship.name}!`
        );
        drawShip(ship.name, ship.startPosX, ship.startPosY, ship.isVertical);
      }
    } else {
      setInfoContainerText(
